...
export default function ClaimPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/tx-confirm?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
        ...
        if (data.transaction.type !== "claim") {
          setError("This token is for a withdrawal, not a claim. Use the correct link.");
          setFlow("error");
          return;
        }
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      // Connect wallet — skip the popup if already connected
  ...
  // No MetaMask
  if (flow === "no_metamask") {
    const confirmUrl = `https://ospex-mini-app.vercel.app/claim?token=${encodeURIComponent(tokenRef.current)}`;
    ...
    return (
      <div style={containerStyle}>
        <div style={cardStyle}>
          ...
          >
            Review and sign to claim your winnings
          </p>
          ...
          <div style={detailRowStyle}>
            ...
            <span>{txDetails.txParams.method === "claimPosition" ? "Claim winnings" : txDetails.txParams.method}</span>
          </div>
          <div style={detailRowStyle}>
  ...
  if (flow === "submitting" || flow === "posting") {
    const statusMessages: Record<string, string> = {
      submitting: "Confirm claim in MetaMask...",
    ...
    return (
      <div style={containerStyle}>
        <div style={{ ...cardStyle, textAlign: "center" }}>
          ...
          >
            Your claim went through on-chain but we couldn&apos;t record it.
